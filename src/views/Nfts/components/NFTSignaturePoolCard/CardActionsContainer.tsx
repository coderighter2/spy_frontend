...
import { useNFTBalances, useNFTSignatureRewardAllowance } from 'state/nft/hooks'
...
const CardActions: React.FC<NFTCardActionsProps> = ({ account, earnings, nextHarvestUntil }) => {
  ...
  const nftBalances = useNFTBalances(tokens.signature.address)
  const allowance = useNFTSignatureRewardAllowance()
  ...
  const { onApproveGeneralReward: onApprove } = useApproveGeneralReward(tokens.signature.address)
